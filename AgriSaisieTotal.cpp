...
//        n=occc->Ex-1;
//        n-=2000;//!!!!!!                130303
		resultat->FraisOccc[r]=occc->Montant*(occc->Pcent/100.)*(occc->Taux/100.);
	}
}
//---------------------------------------------------------------------------
...
//    memset(resultat->SubResid,   0,sizeof(float)*(N_X+1));
	...
	for( r=0;r<nbelt;r++)
	{
		...
		sub->Calcul(Agri->DefSim->An_0,amor,vResid);
		n=sub->ExReal;
		if(n>=0 && n< N_X)
		totSub->Valeur[n]+=sub->Montant;
		for(n=0; n<N_X; n++)
		totSub->Amor[n]+=amor[n];
		for(n=0;n<N_X+1;n++)
		totSub->Resid[n]+=vResid[n];
...
void __fastcall TAgriSaisieFrm::TotalPlact(void)
{
	...
	for(int i=0;i<Agri->V_Plact->Nbelt;i++)
	{
		...
		plact->Calcul (Agri->DefSim->An_0,Agri->DefSim->MDeb,capital,remb,interet,dette);
		if(plact->Pour==CarEnt)
		{
			totPlactE->EstVide=false;
			for(n=0;n<N_X;n++)
			{
				totPlactE->Capital[n]+= capital[n];
				//creance
				totPlactE->Dette[n]+= dette[n];
				totPlactE->Remb[n]+= remb[n];
				//interet
				totPlactE->Interet[n]+= interet[n];
			}
		}
		else //Famille
		{
			totPlactP->EstVide=false;
			for(n=0;n<N_X;n++)
			{
				totPlactP->Capital[n]+= capital[n];
				//creance
				totPlactP->Dette[n]+= dette[n];
				//interet versé annuellement
				if(plact->Type==CarPlactAn)
				totPlactP->Interet[n]+= interet[n];
			}
		}
	}
}
...
void __fastcall TAgriSaisieFrm::TotalFinSom(void)
{
	...
	for(j=0;j<N_X+1;j++)totEmpLT->Dette[j]=fN_Xp1->Valeur[j];

	i=1; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totEmpLT->Remb[j]=fN_Xp1->Valeur[j];

	i=2; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totEmpLT->Interet[j]=fN_Xp1->Valeur[j];

	i=3; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totEmpLT->Capital[j]=fN_Xp1->Valeur[j];

	//Prive
	i=4; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X+1;j++)totEmpCT->Dette[j]=fN_Xp1->Valeur[j];

	i=5; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totEmpCT->Remb[j]=fN_Xp1->Valeur[j];

	i=6; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totEmpCT->Interet[j]=fN_Xp1->Valeur[j];

	i=7; fN_Xp1=vecteur->Vecteur[i];
...
{
	CL_AgriRes *resultat;
	CL_TotEmp *totGene,*totEmp,*totEmpA;
	...
	totEmpA=resultat->EmpLTE;
	totEmp=resultat->EmpSomLT;
	if(totEmp->EstVide==false)
	totEmpA->Ajoute(totEmp->Capital,totEmp->Remb,totEmp->Interet,totEmp->Dette);
...
//---------------------------------------------------------------------------
void __fastcall TAgriSaisieFrm::TotaliserEmpPrive (void)
{
	CL_AgriRes *resultat;
	CL_TotEmp *totGene,*totEmp; //,*totEmpA;
	...
	totGene=resultat->EmpTotalPrive;
	totGene->Maz();//met EstVide à true

	totEmp=resultat->EmpLTP;
	if(totEmp->EstVide==false)
	totGene->Ajoute(totEmp->Capital,totEmp->Remb,totEmp->Interet,totEmp->Dette);

	totEmp=resultat->EmpCTP;
	if(totEmp->EstVide==false)
	totGene->Ajoute(totEmp->Capital,totEmp->Remb,totEmp->Interet,totEmp->Dette);
...
//float delta;
	...
	for(i=0;i<Agri->V_Immo->Nbelt;i++)
	{
		...
		immo->Calcul (Agri->DefSim->An_0,amor,vResid,exAchat,exVente);
		//160102
		n1=exAchat;if(n1<0) n1=0;
		n2=exVente;
		if(n2<0) n2=N_X;//date absente
		if(n2>N_X)n2=N_X;
		for(n=n1;n<n2;n++)totImmo->ValBrut[n]=+immo->ValAchat;
		...
		if(exVente>=0 && exVente<N_X)
		{
			...
			totImmo->ValCompta[exVente]+=immo->ValComptable;
			/*
...
void __fastcall TAgriSaisieFrm::TotalImmoPetit (void)
{
	...
	for(i=0;i<Agri->V_Petit->Nbelt;i++)
	{
		...
		totImmo->Achat[i] +=petit->ValAchat;
		if (petit->Tva !=NULL)
		totImmo->TvaVersee[i]+=petit->ValAchat*(petit->Tva->Taux/100.);
		totImmo->Amor[i] +=petit->ValAchat;
	}
}
//---------------------------------------------------------------------------
...
// tva à calculer
	...
	i=1; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->Amor[j]=fN_Xp1->Valeur[j];//140303

	i=2; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->Achat[j]=fN_Xp1->Valeur[j];

	i=3; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->Vente[j]=fN_Xp1->Valeur[j];

	//val comptable=val vente - plusValue ou  + moinValue
	i=4; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->ValCompta[j]=
	totImmo->Vente[j]
	-fN_Xp1->Valeur[j];//plus Value;

	i=5; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->ValCompta[j]+=fN_Xp1->Valeur[j];//moins Value

	i=6; fN_Xp1=vecteur->Vecteur[i];
	for(j=0;j<N_X;j++)totImmo->ProdImmo[j]=fN_Xp1->Valeur[j];
	//200902
	for(j=0;j<N_X;j++)totImmo->TvaVersee[j]=totImmo->Achat[j]*(TvaDefaut->Taux/100.);
	for(j=0;j<N_X;j++)totImmo->TvaRecue [j]=totImmo->Vente[j]*(TvaDefaut->Taux/100.);
...
void __fastcall TAgriSaisieFrm::TotaliserImmo (void)
{
	...
	for(i=0;i<N_X;i++)
	{
		...
		totGene->ValCompta[i]+=resultat->Immo->ValCompta[i];
		totGene->ValCompta[i]+=resultat->ImmoSom->ValCompta[i];
		...
		// ? voir pour les entrées sommaires
		//pet etre ajouter une ligne dans entrées
		totGene->ValBrut[i]=resultat->Immo->ValBrut[i];
	}
	for(i=0;i<N_X+1;i++)
	{
...
//---------------------------------------------------------------------------
void __fastcall TAgriSaisieFrm::TotalEnCours(CL_Agriculteur *agri)
{
	int aPaie; //171103
	int i,j,n;
	CL_AgriRes *resultat;
	CL_TotEnCours *total;
	CL_Vecteur<CL_EnCours> *vecteur;
	CL_EnCours *enCours;
	float *totTva;
	float tva;
	resultat=agri->Resultat;
	for(i=0;i<2;i++)
	{
		if(i==0) //Creance
		{
			vecteur=agri->V_Creance;
			total=resultat->Creance;
			totTva=resultat->Tva->Recu;
		}
		else //Dette
		{
			vecteur=agri->V_Dette;
			total=resultat->Dette;
			totTva=Agri->Resultat->Tva->Verse;
		}
		total->Maz();
		if(vecteur->Nbelt) total->EstVide=false;

		for(j=0;j<vecteur->Nbelt;j++)
		{
			enCours=vecteur->Vecteur[j];
			aPaie= CalculExercice(enCours->APaie,enCours->MPaie,Agri->DefSim->An_0,Agri->DefSim->MDeb);
			if(aPaie>=0 && aPaie<N_X)
			{
				total->Flux[aPaie]+=enCours->Valeur;
				tva=enCours->Valeur*enCours->Item->Tva->Taux/100;
				totTva[aPaie]+=tva;
			}
			//penser à consommer
			for(n=0;n<aPaie+1;n++)
			total->CrDette[n]+=enCours->Valeur;
		}
	}
}
//-----------------------------------------------------------------------
int __fastcall TAgriSaisieFrm:: CalculExercice(int aReal,int mReal,int an_0,int mois_0)
{
	int n;
