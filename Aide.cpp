...
//HWND H;
	...
	//*********************
	//H = GetDesktopWindow();
	//m_hWindow = Mon_HtmlHelp(H, AsNomFichAide.c_str(),HH_HELP_CONTEXT, HelpContextId);
	FHelpSystemHook->HelpContext(HelpContextId);
}
//---------------------------------------------------------------------------
...
//        	AsNomFichAide= ExtractFilePath(ParamStr(0)) + "olympe.chm";break;
	...
	FHelpSystemHook = new THookHelpSystem(AsNomFichAide, "", htHHAPI);

	if (HHCtrlHandle == 0)
		ShowMessage("HTML Help n'est pas installé sur ce PC");
	else
		HtmlHelp(NULL, NULL, HH_INITIALIZE, (DWORD) m_Cookie);
...
void TAideFrm::Fermer(void) {
	 ...
	 */
	if (HHCtrlHandle != 0)
		HtmlHelp(NULL, NULL, HH_UNINITIALIZE, (DWORD) m_Cookie);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//------------------------------------------------------------------------------
/*bool TAideFrm::ChargerHtmlHelp(void)
 ...
 if (HHLibrary != 0)
 {
 Mon_HtmlHelp = (HTML_HELP_PROC) ::GetProcAddress(HHLibrary, "HtmlHelpA");
