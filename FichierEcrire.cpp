...
/////////////!!!!!!!!!!!
	...
	if (fichier == NULL) {
		MessageBox(0, " Pas de fichier OU fichier protégé en écriture",
	...
	if (Version >= 132)
		fwriteString(MainForm->Notes); //020507
	// SYSTEME UNITE///////////////////////
	EcrireSystemeUnite();
	EcrireTva();
	// produits charges
	...
	EcrireBestiaux(V_Bestiaux); //190802
	...
	if (Version >= 134)
		EcrireEtatSortie134(V_EtatSortie);
	else
		EcrireEtatSortie130(V_EtatSortie);

	EcrireCategorie(V_CatSerieComp);
	if (Version >= 134)
		EcrireEtatSortie134(V_SerieComp);
	else
		EcrireEtatSortie130(V_SerieComp);

//	EcrireCategorie(V_CatProcedure);     A INTRODUIRE proc
//  EcrireEtatSortie(V_Procedure);
	...
	// agriculteur
	if (Version >= 132)
		fwriteString(GereAgriFrm->Notes); //020507
...
//RUSTINE !!!!!!!!!
	...
	if (Version >= 132)
		fwriteString(SysUniteFrm->Notes);
...
//-------------------------------------------------------------------
void CL_ES::EcrireBestiaux(CL_Vecteur<CL_Bestiaux> *vecteur) {
	...
	for (i = 0; i < nbelt; i++) {
		...
		bestiaux->Flag = 'O';
		fwrite(&bestiaux->No, sizeof(int), 1, fichier);
		fwriteString(bestiaux->Nom);
		fwrite(&bestiaux->NoCategorie, sizeof(int), 1, fichier);
		fwrite(&bestiaux->NoOrigine, sizeof(int), 1, fichier); //190802

		fwrite(&bestiaux->NoTva, sizeof(int), 1, fichier);
		fwrite(&bestiaux->Prix, sizeof(float), 1, fichier);
		fwrite(&bestiaux->ValInv, sizeof(float), 1, fichier); //081003
		for (n = 0; n < 4; n++)
			fwrite(&bestiaux->DonneNo[n], sizeof(int), 1, fichier);
		for (int n = 0; n < 4; n++)
			fwrite(&bestiaux->DonnePcent[n], sizeof(float), 1, fichier);
	}
...
void CL_ES::EcrireAtelier(CL_Vecteur<CL_Atelier> *vecteur) {
	...
	for (int i = 0; i < nbelt; i++) {
		...
		//piedHa  290307
		if (Version >= 131) {
			fwrite(atelier->PiedHa, sizeof(float), atelier->NbPhase, fichier);
			EcrireAtelierItem(atelier->V_ProdPied, atelier->NbPhase);
			EcrireAtelierItem(atelier->V_ChPied, atelier->NbPhase);
		}
...
void CL_ES::EcrireEspece(void) {
	...
	for (int i = 0; i < nbelt; i++) {
		...
		fwrite(espece->Phase, sizeof(int) * nPhase, 1, fichier); //180407
	}
}
//-------------------------------------------------------------------------
...
//		fwrite(&invAni->ValInv, sizeof(float),1,fichier);   //260903  supprime 0031003
	...
	for (i = 0; i < lstMvtAni->Count; i++) {
		...
		fwrite(&mvtAni->Ligne, sizeof(int), 1, fichier);
...
void CL_ES::EcrireAgriTroupeau(TList *lstTroupeaux) {
	...
	for (n = 0; n < lstTroupeaux->Count; n++) {
		...
		//TroupeauFrm->EcritLstMvtAni(troupeau->LstMvtAni);test
...
//-------------------------------------------------------------------------
bool CL_ES::EcrireAgriSub(CL_Vecteur<CL_Sub> *vecteur) {
	CL_Sub* sub;
	int nbelt, noX;
	bool flag; //190412
	nbelt = vecteur->Nbelt;
...
//-------------------------------------------------------------------------
void CL_ES::EcrireEtatSortie130(CL_Vecteur<CL_EtatSortie> *v_Etat) {
	int l;
	CL_EtatSortie *etat;
	CL_Elt *elt;
...
//-------------------------------------------------------------------------
void CL_ES::EcrireEtatSortie134(CL_Vecteur<CL_EtatSortie> *v_Etat) {
	int l;
	CL_EtatSortie *etat;
	CL_Elt *elt;
	CL_Vecteur<CL_Elt> *vecteur;
	...
	for (int i = 0; i < nbelt; i++) {
		...
		for (int j = 0; j < vecteur->Nbelt; j++) {
			...
			fwrite(&elt->NoItem, sizeof(int), 1, fichier);
			if (Version >= 134) {
				fwrite(&elt->Digits, sizeof(int), 1, fichier);
				fwrite(&elt->Coul, sizeof(TColor), 1, fichier);
			}
			if (elt->NoItem == TITRE) {
				l = strlen(elt->Nom) + 1;
				fwrite(&l, sizeof(int), 1, fichier);
...
void CL_ES::EcrireTendance(CL_Vecteur<CL_Tendance> *vecteur) {
	...
	for (int i = 0; i < nbelt; i++) {
		...
		if (Version >= 132)
			fwriteString(tendance->Notes); //020507
		...
		for (int j = 0; j < vDelta->Nbelt; j++) {
			...
			for (int k = 0; k < delta->NMax; k++)
				fwrite(&delta->Pcent[k], sizeof(float), 1, fichier);
		}
	}
}
//-------------------------------------------------------------------------
void CL_ES::AffLstMvtAniCount(void) {
	int i, j, n;
	CL_Agriculteur *agri;
	CL_Troupeau *troupeau;
	char buf[100];
	for (i = 0; i < V_Agriculteur->Nbelt; i++) {
		...
		for (j = 0; j < agri->LstTroupeaux->Count; j++) {
			troupeau = (CL_Troupeau*) agri->LstTroupeaux->Items[j];

			n = troupeau->LstMvtAni->Count;
			sprintf(buf, "i=%d  j=%d n=%d", i, j, n);
			ShowMessage(buf);
		}
	}
}
