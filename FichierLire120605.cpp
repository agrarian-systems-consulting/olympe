...
///        2               Abandon
//---------------------------------------------------------------------------
int CL_ES::lireFichier(bool tout) {
	int rep;
	int zero;
...
//bool impFichier=true;
	...
	if (fichier == NULL) {
		as = "";
		as.cat_sprintf(L_ExistePas.c_str(), FEcriture.c_str());
		//sprintf(bufAncien,"Le Fichier %s n'existe pas\n Le créer ?",FEcriture.c_str());
		//rep=MessageBox(MainForm->Handle,bufAncien,"PROBLEME SUR FICHIER",MB_YESNO|MB_ICONEXCLAMATION);
		rep = MessageBox(MainForm->Handle, as.c_str(), L_ProbSurFichier.c_str(),
	...
	if (version != Version) {
		if (version > 100 && version < Version) {
			as = "";
			as.cat_sprintf(L_VerActuelle.c_str(), Version / 100.);
			//sprintf(bufEnCours,"Actuellement Version= %4.2f",Version/100.);
			as1 = "";
			as1.cat_sprintf(L_JmetAJour.c_str(), version / 100.);
			//sprintf(bufAncien,"Je mets à jour Version= %4.2f  ?",version/100.);

			rep = MessageBox(MainForm->Handle, as1.c_str(), as.c_str(),
					MB_YESNO | MB_ICONSTOP);
			//rep=MessageBox(MainForm->Handle,bufAncien,bufEnCours,MB_YESNO|MB_ICONSTOP);
			if (rep == IDNO) {
				fclose(fichier); //210907
				return 2;
			}
			mettreAJour = true;
		} else {
			as = "";
			as.cat_sprintf(L_VerInco.c_str(), version / 100.);
			//sprintf(bufEnCours,"Version= %4.2f   Inconnue",version/100.);
			//MessageBox(MainForm->Handle,bufEnCours,"Problème sur Fichier",MB_OK|MB_ICONSTOP);
			MessageBox(MainForm->Handle, as.c_str(), L_ProbSurFichier.c_str(),
					MB_OK | MB_ICONSTOP);
			fclose(fichier); //181207
			return 2;
		}
	}
	if (version >= 132) //020507
		freadString(MainForm->Notes);
...
//	LireCategorieP(V_CatProduit);
	LireCategorie(V_CatProduit, version);
	LireItem(V_Produit, V_CatProduit);

	LireCategorie(V_CatCharge, version);
	LireItem(V_Charge, V_CatCharge);

	// V_CatExternalite est standard cf main
	LireItem(V_Externalite, V_CatExternalite);

	if (version >= 123) {
		LireCategorie(V_CatBestiaux, version); //101201
		if (version <= 126)
			//erreur possible LireItem(V_Bestiaux)
			LireBestiaux127(V_Bestiaux, V_CatBestiaux); //101201
		else if (version == 127)
			LireBestiaux127(V_Bestiaux, V_CatBestiaux); //101201
		else
			LireBestiaux(V_Bestiaux, V_CatBestiaux); //101201
	}
	if (version < 130)
		MajV_Bestiaux(V_Bestiaux);

	LireCategorie(V_CatCulture, version);
	LireCategorie(V_CatAnimaux, version);
	LireCategorie(V_CatVivrier, version);
	if (version >= 108) {
		LireCategorie(V_CatChStruct, version);
		LireCategorie(V_CatRecDiv, version);
		LireCategorie(V_CatDepDiv, version);
		LireCategorie(V_CatRecFam, version);
		LireCategorie(V_CatDepFam, version);
	...
	//Atelier
	if (version <= 119) {
		LireAtelier119(V_Culture, V_CatCulture);
		LireAtelier119(V_Animaux, V_CatAnimaux);
		LireAtelier119(V_Perenne, V_Espece);
		LireAtelier119(V_Vivrier, V_CatVivrier);
	} else if (version <= 122) {
		LireAtelier122(V_Culture, V_CatCulture);
		LireAtelier122(V_Animaux, V_CatAnimaux);
		LireAtelier122(V_Perenne, V_Espece);
		LireAtelier122(V_Vivrier, V_CatVivrier);
	} else {
		LireAtelier(V_Culture, CULTURE, V_CatCulture, version);
		LireAtelier(V_Animaux, ANIMAUX, V_CatAnimaux, version);
		LireAtelier(V_Perenne, PERENNE, V_Espece, version);
		LireAtelier(V_Vivrier, VIVRIER, V_CatVivrier, version);
	}
	//V_Vivrier->purge();//  !!!!
//....
	if (version >= 109)
		LireItem(V_ChStruct, V_CatChStruct);

	LireItem(V_RecDiv, V_CatRecDiv);
	LireItem(V_DepDiv, V_CatDepDiv);
	LireItem(V_RecFam, V_CatRecFam);
	LireItem(V_DepFam, V_CatDepFam);
	//140408
	for (int n = 0; n < V_RecFam->Nbelt; n++) {
		item = V_RecFam->Vecteur[n];
		item->Tva = TvaDefaut;
		item->NoTva = TvaDefaut->No;
	}
	for (int n = 0; n < V_DepFam->Nbelt; n++) {
		item = V_DepFam->Vecteur[n];
		item->Tva = TvaDefaut;
		item->NoTva = TvaDefaut->No;
	}
	if (version < 108) {
	...
	// types zone
	LireCategorie(V_CatTypeZone, version);
	LireItem(V_TypeZone, V_CatTypeZone);
	//V_CatTypeZone->purge();  //220207
	//V_TypeZone->purge();
	if (version <= 113)
	...
	if (version <= 120) {
		LireCategorie(V_CatEtatSortie, version);
		LireEtatSortie(V_EtatSortie, V_CatEtatSortie, version);
	} else {
		// etatSortie
		LireCategorie(V_CatEtatSortie, version);
		LireEtatSortie(V_EtatSortie, V_CatEtatSortie, version);
		// comparaison utilise les mêmes objets
		LireCategorie(V_CatSerieComp, version);
		LireEtatSortie(V_SerieComp, V_CatSerieComp, version);

//    	LireCategorie (V_CatProcedure);               proc
//      LireEtatSortie(V_Procedure,V_CatProcedure);
	}

	LireCategorie(V_CatIndic, version);
	if (version <= 106)
		LireIndicateur106(V_CatIndic);
	else
		LireIndicateur(V_CatIndic, version);
	LireCategorie(V_CatVar, version);
	LireItem(V_Variable, V_CatVar);

	LireCategorie(V_CatDico, version);
	LireItem(V_Dico);
	if (version >= 106) {
		LireTendance(V_TendPrixProd, V_Produit, version);
		LireTendance(V_TendPrixCharge, V_Charge, version);
		LireTendance(V_ScenPrixProd, V_Produit, version);
		LireTendance(V_ScenPrixCharge, V_Charge, version);
	} else {
	...
	if (version >= 108) {
		LireTendance(V_TendQProd, V_Produit, version);
		LireTendance(V_TendQCharge, V_Charge, version);
		LireTendance(V_ScenQProd, V_Produit, version);
		LireTendance(V_ScenQCharge, V_Charge, version);
		LireTendance(V_TendQExt, V_Externalite, version);
		LireTendance(V_ScenQExt, V_Externalite, version);
	} else {
		V_TendQProd->purge();
		V_TendQCharge->purge();
		V_ScenQProd->purge();
	...
	if (version >= 120)
		LireConversion();
	if (version >= 129) {
		LireCategorie(V_CatRepart, version);
		LireRepart(V_Repart);
	}
	...
	//possible de recupérer les données globales

	if (tout == true) {
		// agriculteur
		if (version >= 132)
			freadString(GereAgriFrm->Notes);

		LireAgriculteur();

		// Ensemble d'agriculteurs à simuler

		LireEnsemble(version);

		if (version >= 111)
			freadString(Notes);
		fread(&zero, sizeof(int), 1, fichier);
...
//----------------------------------------------------------------------------
void CL_ES::LireSystemeUnite(int version) {
	int nbelt;
	float f;
	int noMonnaie;
	CL_SystemeUnite *item;
...
//----------------------------------------------------------------------------
void CL_ES::LireCategorie(CL_Vecteur<CL_Categorie> *vecteur, int version) //020507
		{
	int nbelt;
...
/*
 ...
 fread(&noCat,sizeof(int),1,fichier);
 categorie=V_CatTypeZone->trouve(noCat);
 if(categorie==NULL)   continue;  //060602??
...
void CL_ES::LireItem(CL_Vecteur<CL_Item> *vecteur,
	...
	for (int i = 0; i < nbelt; i++) {
		...
		if (item->Categorie == NULL) //230106
				{
			item->Categorie = vCategorie->Vecteur[0];
			item->NoCategorie = item->Categorie->No;

		}

		item->SysUnite = V_SystemeUnite->trouve(item->NoSysteme);
		if (item->SysUnite == NULL) //040306
				{
		...
		if (item->Categorie == NULL)
			continue; //rustine 0801204

		item->Categorie->Supprimable = false;
		if (item->SysUnite)
			item->SysUnite->Supprimable = false;
		if (item->Tva)
			item->Tva->Supprimable = false;
		nb++;
		vecteur->insSans(item);
	}
	vecteur->Nbelt = nb; //rustine 0801204
}

//-------------------------------------------------------------------
void CL_ES::LireBestiaux127(CL_Vecteur<CL_Bestiaux> *vecteur,
		CL_Vecteur<CL_Categorie> *vCategorie) {
	int i;
	int nbelt;
	...
	for (i = 0; i < nbelt; i++) {
		...
		fread(&bestiaux->No, sizeof(int), 1, fichier);
		freadString(bestiaux->Nom);
		//correspond à famille taxes NE Plus les stocker ni les lire
		fread(&bestiaux->NoCategorie, sizeof(int), 1, fichier);
		fread(&bestiaux->NoOrigine, sizeof(int), 1, fichier); //190802
		fread(&bestiaux->NoTva, sizeof(int), 1, fichier);
		fread(&bestiaux->Prix, sizeof(float), 1, fichier);

		bestiaux->Categorie = vCategorie->trouve(bestiaux->NoCategorie);
		bestiaux->Tva = V_Tva->trouve(bestiaux->NoTva);

		if (bestiaux->Categorie == NULL)
			continue; //rustine 0801204

		bestiaux->Categorie->Supprimable = false;
		bestiaux->Tva->Supprimable = false;

		vecteur->insSans(bestiaux);
		nb++;
	}
	vecteur->Nbelt = nb; //rustine 0801204
}
//-------------------------------------------------------------------
void CL_ES::LireBestiaux(CL_Vecteur<CL_Bestiaux> *vecteur,
		CL_Vecteur<CL_Categorie> *vCategorie) {
	int i, n;
	int nbelt;
	int nb;
	...
	for (i = 0; i < nbelt; i++) {
		...
		fread(&bestiaux->NoOrigine, sizeof(int), 1, fichier); //190802
		fread(&bestiaux->NoTva, sizeof(int), 1, fichier);
		fread(&bestiaux->Prix, sizeof(float), 1, fichier); //081003
		if (version >= 130)
			fread(&bestiaux->ValInv, sizeof(float), 1, fichier); //091003
		//160103
		for (n = 0; n < 4; n++)
			fread(&bestiaux->DonneNo[n], sizeof(int), 1, fichier);
		for (n = 0; n < 4; n++)
			fread(&bestiaux->DonnePcent[n], sizeof(float), 1, fichier);
...
//------------------------------------------------------------------------------
void CL_ES::MajV_Bestiaux(CL_Vecteur<CL_Bestiaux> *vecteur) {
	int i;
	CL_Bestiaux *bestiaux;
	for (i = 0; i < vecteur->Nbelt; i++) {
		bestiaux = vecteur->Vecteur[i];
		bestiaux->ValInv = bestiaux->Prix;
	}
}
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
void CL_ES::LireAtelier119(CL_Vecteur<CL_Atelier> *vecteur,
		CL_Vecteur<CL_Espece> *vCategorie) {
	int nbelt;
	CL_Atelier *atelier;
	CL_Espece *espece;
	vecteur->purge();
...
void CL_ES::LireAtelier(CL_Vecteur<CL_Atelier> *vecteur, int nature,
	...
	for (int i = 0; i < nbelt; i++) {
		...
		//piedHa  290307
		if (version >= 131) {
			fread(atelier->PiedHa, sizeof(float), atelier->NbPhase, fichier);
			LireAtelierItem(atelier->V_ProdPied, atelier->NbPhase);
			LireAtelierItem(atelier->V_ChPied, atelier->NbPhase);
		} else
			memset(atelier->PiedHa, 0, sizeof(float) * atelier->NbPhase);
		...
		//correction     erreur 030804
		if (atelier->DebAmor == 0 || atelier->DurAmor == 0) {
			espece = V_Espece->trouve(atelier->NoEspece);
			if (espece) {
				atelier->DebAmor = espece->DebAmor;
				atelier->DurAmor = espece->DurAmor;
...
//-------------------------------------------------------------------------
void CL_ES::LireAtelier(CL_Vecteur<CL_Atelier> *vecteur, int nature,
		CL_Vecteur<CL_Espece> *vCategorie, int version) {
	int nbelt;
	CL_Atelier *atelier;
	CL_Espece *espece;
	int derobe;
	...
	for (int i = 0; i < nbelt; i++) {
		...
		if (version >= 126)
			LireAtelierBesTrav(atelier->V_BesTrav);
//atelier->V_BesTrav->Nbelt=0;   //050806
		// debut et durée d'amortissement
		fread(&atelier->DebAmor, sizeof(int), 1, fichier);
		fread(&atelier->DurAmor, sizeof(int), 1, fichier);
		//correction     erreur 030804
...
//070407
//----------------------------------------------------------------------------
void CL_ES::LireEspece(int version) {
	int i;
	int nbelt;
	CL_Espece *espece;
	...
	for (i = 0; i < nbelt; i++) {
		...
		if (version <= 130) //(version==130)            //300511
			fread(espece->Phase, sizeof(int) * 14, 1, fichier);
		else
...
//-------------------------------------------------------------------------
void CL_ES::LireCalendrier(int version) {
	int i, j;
	int nbelt, nbPeriode;
...
//nbelt=3;
	for (i = 0; i < nbelt; i++) {
		...
		for (int j = 0; j < nCritere; j++) {
			...
			if (critere->Flag == 'N') {
				critere->Flag = 'O';
				agri->LstCritere->Add(critere); //220207
			}
		}
		nCritere = 0; //220207
		...
		if (version >= 103) {
			...
			flag = LireAgriSub(agri->V_Sub);
			if (flag == true) {
				ErreurSub = true;
				AnsiString as;
				as = "Lecture  " + agri->Nom + "  " + "Subvention !?";
			 ...
			 */
			if (agri->V_Petit->Nbelt != N_X) //inutile
			{ //040507
				CL_Petit *petit;
				agri->V_Petit->purge();
				for (int n = 0; n < N_X; n++) {
...
///*.....   A METTRE
		//simu de + 10 ans   251006
		// je cree une LstSerie comparable à LstVariante qui contient
		//des ST_NoAgri  No de l'agricultreur  Agri Adresse
		//   renseigné dans  FichierAdresse CL_ES::adresseAgri
		// pour ne pas modifier la structure du fichier je n'ai pas stocké
		//le nombre
		// mais je peux le calculer  agri->DefSim->NbAnSim /10;
		if (agri->DefSim->NbAnSim > 10) {
			int nSerie;
			nSerie = agri->DefSim->NbAnSim / 10;
			agri->LstSerie = new TList;
			for (int j = 0; j < nSerie; j++) {
				variante = new (ST_NoAgri);
				fread(&variante->No, sizeof(int), 1, fichier);
				agri->LstSerie->Add(variante);
			}
		}
//*/
...
//-------------------------------------------------------------------------
void CL_ES::VerifNumeroAgri(void) {
	CL_Agriculteur *agri;
	int noDonne[5000];
	int n, nd;
	bool probleme = false;
	...
	V_Agriculteur->NoX = noX + 1;
	if (probleme == false)
		return;

	rep = MessageBox(MainForm->Handle, "numerotation agriculteurs A Corriger",
			"PROBLEME SUR FICHIER", MB_YESNO | MB_ICONEXCLAMATION);
	if (rep == ID_NO)
		return;
...
void CL_ES::LireAgriValeurItem(CL_Vecteur<CL_Valeur> *vecteur) {
	...
	for (int i = 0; i < nbelt; i++) {
		...
		valeur->Atelier = NULL;
		valeur->Item = V_Variable->trouve(valeur->NoItem);
		vecteur->insSans(valeur);
	}
}
//-------------------------------------------------------------------------
...
bool CL_ES::LireAgriSub(CL_Vecteur<CL_Sub> *vecteur) {
	CL_Sub* sub;
	int nbelt;
	bool flag = false;
	AnsiString as;
	...
	for (int i = 0; i < nbelt; i++) {
		...
		//testSub 190412
		if (sub->Montant > 100000) {
			flag = true;
			sub->Montant = (i + 1) * 1000;
		}
...
void CL_ES::LireAgriOccc(CL_Vecteur<CL_Occc> *vecteur) {
	...
	if (vecteur->Nbelt > N_X)
		vecteur->Nbelt = N_X; //160105
...
//-------------------------------------------------------------------------
void CL_ES::LireEnsemble(int version) {
	int i, j, n;
	int nbelt;
	int nbAgriEff;
...
//AnsiString nom;//!!
	...
	for (i = 0; i < nbelt; i++) {
		...
		for (j = 0; j < nbeltV; j++) {
			...
			if (version >= 134) {
				fread(&elt->Digits, sizeof(int), 1, fichier);
				fread(&elt->Coul, sizeof(TColor), 1, fichier);
			}
			if (elt->NoItem == TITRE) {
...
//----------------------------------------------------------------------------
void CL_ES::LireIndicateur106(CL_Vecteur<CL_Categorie> *vCategorie) {
	int nbelt;
...
//----------------------------------------------------------------------------
void CL_ES::LireIndicateur(CL_Vecteur<CL_Categorie> *vCategorie, int version) {
	int nbelt;
	CL_Indicateur *indicateur;
	V_Indicateur->purge();
...
///*
		...
		if (Arbre->LesBranches[n].code == IDENT
				&& Arbre->LesBranches[n].arg[0] == MVT) {
			if (Arbre->LesBranches[n].arg[1] < EFF) //160     //281111
				Arbre->LesBranches[n].arg[1] = EFFV; //161;
		}

//*/
//supprimable à mettre ici ???
...
void CL_ES::LireTendance(CL_Vecteur<CL_Tendance> *vecteur,
		CL_Vecteur<CL_Item> *vItem, int version) {
	int nbelt, nbDelta;
	...
	for (int i = 0; i < nbelt; i++) {
		...
		if (version >= 132)
			freadString(tendance->Notes); //020507
		...
		for (int j = 0; j < nbDelta; j++) {
			...
			//passage à version 133 et +         //210907
			if (ES->Version < 133)
				nMax = N_X;
			else {
				if (version >= 133)
					...
					nMax = N_X;
				//je n'en lis que 10 les utrs ont été mis à 100 par le constructeur
			}
			for (int k = 0; k < nMax; k++)
				fread(&delta->Pcent[k], sizeof(float), 1, fichier);
//            delta->NMax=100;   //191207
			//le constructeur definit delta->Pcent[100]
			//depuis le  210907
			if (erreur == false)
...
/*
 une fois trouvé le début de la suite ie calendrier
 char car;
 char *ptr;
 long curpos;
 int n;
 curpos=ftell(fichier);
 while(1)      //pour trouver la suite
 {
 memset(chaine,0,sizeof(chaine));
 ptr=fgets(chaine,100,fichier);
 }
 // une fois trouvé   position ie 207L
 fseek(fichier,207L,SEEK_CUR);
 curpos=ftell(fichier);
 ptr=fgets(chaine,100,fichier);
...
void CL_ES::MajInvAni(void) {
	...
	for (i = 0; i < V_Animaux->Nbelt; i++) {
		...
		bestiaux = new CL_Bestiaux();
		bestiaux->No = atelier->No;
		bestiaux->Nom = atelier->Nom;
		bestiaux->NoCategorie = cat->No;
		bestiaux->Categorie = cat;
		bestiaux->Tva = TvaDefaut;
		bestiaux->NoTva = TvaDefaut->No;
		if (bestiaux->No > noX)
			noX = bestiaux->No;
