...
// Construire le nom du fichier.Txt */
	...
	as = "Envoi dans fichier:  " + AnsiString(sortieNom);
	ShowMessage(as);
...
void CL_ES::ImpLesEtatSortie(ofstream &outfile) {
	...
	for (int i = 0; i < vecteur->Nbelt; i++) {
		...
		for (int j = 0; j < v_Elt->Nbelt; j++) {
			...
			switch (elt->NoItem) {
			case TITRE:
				sprintf(buf, "TITRE %2d   %2d %2d %2d", elt->No, elt->NoFamille,
						elt->NoCategorie, elt->NoItem);
				break;
			case TRAIT:
				sprintf(buf, "TRAIT %2d   %2d %2d %2d", elt->No, elt->NoFamille,
						elt->NoCategorie, elt->NoItem);
				break;
			case BLANC:
				sprintf(buf, "BLANC %2d   %2d %2d %2d", elt->No, elt->NoFamille,
						elt->NoCategorie, elt->NoItem);
				break;
			default:
...
//-------------------------------------------------------------------------
void CL_ES::ImpLesAgri(ofstream &outfile) {
	int n;
	CL_Agriculteur *agri;
...
//        if(n>=45) agri->No=62+(n-45);
		ImpListeAgri(outfile, n, agri);
	}
	outfile << endl;
	outfile << endl;
}
//-------------------------------------------------------------------------
void CL_ES::ImpListeAgri(ofstream &outfile, int n, CL_Agriculteur *agri) {
	outfile << n << "  " << agri->No << "  " << agri->Nom << endl;
