...
CL_Agriculteur* __fastcall TAgriSaisieFrm::Agrege(CL_Ensemble *ensemble)
{
	...
	CL_Agriculteur *agri;
	CL_AgriEff *agriEff;
	float ponderation[N_X+1]; //280110
	int effectifTot[N_X];
	int nbAgri;
	bool correct;
	AnsiString nom0;
	...
	agriTot->Nom=T->L_TotalM+ensemble->Nom;
	//agriTot->Nom=AnsiString("TOTAL")+ensemble->Nom;
	else//MOYENNE
	agriTot->Nom=L_Moy+ensemble->Nom;
...
//    MazNFlagCat();//pour repérer les catégories à imprimer si sortie
	...
	for(i=0;i<ensemble->V_AgriEff->Nbelt;i++)
	{
		...
		ponderation[N_X]=ponderation[N_X-1];
		//for(n=0;n<N_X;n++)  effectifTot[n]+=agriEff->Effectif[n];
	...
	AgregeCompleteFormule(agriTot->V_ChStruct);
	AgregeCompleteFormule(agriTot->V_RecDiv);
	AgregeCompleteFormule(agriTot->V_DepDiv);
	AgregeCompleteFormule(agriTot->V_RecFam);
	AgregeCompleteFormule(agriTot->V_DepFam);
...
//CL_Item      *item;
	...
	for(i=0;i<vecteur->Nbelt;i++)
	{
		...
		if(valeurTot==NULL)
		{
			valeurTot=new CL_Valeur(valeur);
			memset(valeurTot->Valeur,0,sizeof(float)*N_X);
			//201206   ne pas inderer en donnant numero
			vecteurTot->insSans(valeurTot);
		}
		for(n=0;n<N_X;n++)
		valeurTot->Valeur[n]+=valeur->Valeur[n]*ponderation[n];
	}
		...
		CL_Agriculteur *agriTot)
{
	...
	for(i=0;i<vecteurTot->Nbelt;i++)
	{
		...
		item=valeurTot->Item;
		categorie=item->Categorie;
		if(categorie->Nom.AnsiCompareIC(L_Surface)==0) continue;
...
CL_MvtAni* __fastcall TAgriSaisieFrm::TrouveMvtAni(TList *lst,CL_MvtAni *mvtAni0)
{
	...
	for(i=0;i<lst->Count;i++)
	{
		...
		if(mvtAni->Ligne !=mvtAni0->Ligne) continue;
...
//----------------------------------------------------------------------------
void __fastcall TAgriSaisieFrm::AgregeCompleteFormule(CL_Vecteur<CL_Formule> *vecteur)
{
	int i,n;
	CL_Formule *formule;
	for(i=0;i<vecteur->Nbelt;i++)
	{
		formule=vecteur->Vecteur[i];
		for(n=0;n<N_X;n++)
		formule->LesProg[n]=new CL_Prog(formule->Valeur[n]);
	}
}
//----------------------------------------------------------------------------
