...
//    ShowMessage(AsExe+"   "+AsFichier);    

	rep = CreateProcess(AsExe.c_str(), AsFichier.c_str(), 0, 0, false,
			DETACHED_PROCESS, 0, 0, &siStartupInfo, &piProcessInfo);
...
int CL_Procedure::TrValFormule(CL_Agriculteur *agri,
	...
	CL_Formule *formule;
//  ?? ok pour nombre 1200
// mais pour 1200*3 ??? voir
	eltP->Type = 'F';
	for (i = 0; i < vecteur->Nbelt; i++) {
		formule = vecteur->Vecteur[i];
		if (FormuleModifiable(formule) == false)
			continue;
...
//---------------------------------------------------------------------------
bool CL_Procedure::FormuleModifiable(CL_Formule *formule) {
	int n, i;
	CL_Prog *prog;
	CL_Arbre *arbre;
...
bool CL_Procedure::EcrireDonnees(vector<CL_EltProc*> vEltProc) {
	...
	FILE *fichier;
	CL_EltProc *eltP;
	fichier = fopen(AsFichier.c_str(), "w");
	if (fichier == NULL) {
		ShowMessage(
	...
	eltP = vEltProc[0];
	EcrireEnTete(fichier, eltP);
	for (i = 0; i < vEltProc.size(); i++) {
		eltP = vEltProc[i];

		if (eltP->Ensemble) {
			EcrireEnsemble(eltP->Ensemble, fichier);
			continue;
		}
		if (eltP->Valeur == NULL)
			continue;

		EcrireNom(eltP->Agri->Nom, fichier); //1
		EcrireNom(eltP->Nom, fichier); //2
		EcrireNom(eltP->NomCategorie, fichier); //3
		EcrireNom(eltP->NomFamille, fichier); //4
		EcrireNom(eltP->NomNature, fichier); //5
		for (n = 0; n < N_X; n++)
			fprintf(fichier, "%6.2f ;", eltP->Valeur[n]);
		fprintf(fichier, "\n");
	}
	fclose(fichier);
	return true;
...
//---------------------------------------------------------------------------
void CL_Procedure::EcrireEnTete(FILE *fichier, CL_EltProc *eltP) {
	int n;
	CL_Agriculteur *agri = eltP->Agri;
	EcrireNom("Agri", fichier); //1
	EcrireNom("Nom", fichier); //2
	EcrireNom("Categorie", fichier); //3
	EcrireNom("Famille", fichier); //4
	EcrireNom("Nature", fichier); //5
	for (n = 0; n < N_X; n++)
		fprintf(fichier, "%d ;", agri->DefSim->An_0 + n);
	fprintf(fichier, "\n");
}
//---------------------------------------------------------------------------
void CL_Procedure::EcrireNom(AnsiString as, FILE *fichier) {
	as = as.Trim();
	if (as == "")
		fprintf(fichier, "  ;");
	else
		fprintf(fichier, "%s ;", as.c_str());
}
//---------------------------------------------------------------------------
void CL_Procedure::EcrireEnsemble(CL_Ensemble *ensemble, FILE *fichier) {
	int i, j, n;
	CL_AgriEff *agriEff;
	CL_Agriculteur *agri;
	CL_Valeur *valeur;
	...
	for (i = 0; i < ensemble->V_AgriEff->Nbelt; i++) {
		...
		EcrireNom(agriEff->Agri->Nom, fichier);
		EcrireNom(ensemble->Nom, fichier);
		EcrireNom("Effectif", fichier);
		EcrireNom(" ", fichier);
		EcrireNom("Ensemble", fichier);
		for (n = 0; n < N_X; n++)
			fprintf(fichier, "%4d ;", agriEff->Effectif[n]);
		fprintf(fichier, "\n");
	}
	//Assolement
	for (i = 0; i < ensemble->V_AgriEff->Nbelt; i++) {
		...
		for (j = 0; j < agri->V_Culture->Nbelt; j++) {
			...
			EcrireNom(agri->Nom, fichier); //1
			EcrireNom(valeur->Atelier->Nom, fichier); //2
			EcrireNom(valeur->Atelier->Categorie->Nom, fichier); //3
			EcrireNom("Assolement", fichier); //4
			EcrireNom("Production", fichier); //5
			for (n = 0; n < N_X; n++)
				fprintf(fichier, "%6.2f ;", valeur->Valeur[n]);
			fprintf(fichier, "\n");
		}
	}
}
...
//    for(i=0;i<V_EltP->Nbelt;i++)
	for (i = 0; i < vEltProc.size(); i++)

	{

		eltP = vEltProc[i];
//        eltP=V_EltP->Vecteur[i];
		switch (eltP->Type) {
		case ' ':
...
//---------------------------------------------------------------------------
void CL_Procedure::MajFormule(CL_Formule *formule, int n, float val) {
	CL_Prog *prog;
