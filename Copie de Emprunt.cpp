...
void CL_EmpLT::Aff(int an_0) {
	...
	Forme01Frm->Show();
	Forme01Frm->IniGrilleAn("lt", Nom.c_str());
...
void CL_EmpCT::Aff(int an_0) {
	...
	Calcul(an_0, capital, remb, interet, dette);
	Forme01Frm->Show();
	Forme01Frm->IniGrilleAn("ct", Nom.c_str());
...
void CL_Immo::Aff(int an_0) {
	...
	Forme01Frm->Show();
	Forme01Frm->IniGrilleAn("immo", Nom.c_str());
...
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
void CL_EmpLT::RembConstantTreso(float *recu, float *verse) {
	float verst, du, rembt, interest, taux, taux1, taux2;
...
//-------------------------------------------------------------------------
void CL_EmpLT::RembVariableTreso(float *recu, float *verse) {
	float verst, du, rembt, interest, taux1, taux2;
...
//-----------------------------------------------------------------------
void CL_EmpCT::CalculTreso(float *recu, float *verse) {
	float duree;
	char type = Type[1];
	float interest;
//int n0,n1;
...
//-----------------------------------------------------------------------
void CL_Sub::CalculTreso(float *recu, float *verse) {
	int t;
	memset(recu, 0, sizeof(recu));
...
//-----------------------------------------------------------------------
void CL_Plact::CalculTreso(float *recu, float *verse) {
	float duree;
	char type = Type;
	float cap, intAn;
	int n;
...
//-----------------------------------------------------------------------
void CL_Immo::CalculTreso(int &mVente, float &valVente, float &tvaRecu,
		int &mAchat, float &valAchat, float &tvaVerse)

		{
//int t;

//	memset(recu,0,sizeof(float)*N_X*12);
//	memset(verse,0,sizeof(float)*N_X*12);
	mAchat = MoisTreso(AAchat, MAchat);
	...
	mVente = MoisTreso(AVente, MVente);
