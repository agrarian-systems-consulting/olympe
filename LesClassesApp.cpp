...
//---------------------------------------------------------------------------
CL_Bestiaux::CL_Bestiaux(CL_Bestiaux *anc) {
	int n;
	Nom = anc->Nom;
	Categorie = anc->Categorie;
	SysUnite = anc->SysUnite;
...
//---------------------------------------------------------------------------
CL_ItemParAtelier::CL_ItemParAtelier(const CL_ItemParAtelier *ptr) {
	Item = ptr->Item;
	ItemPA = ptr->ItemPA; //pour les charges proportionnelles au volume   no_produit!=-1
	Charge = ptr->Charge;
	No = ptr->No;
	NoItem = ptr->NoItem;
	NoCharge = ptr->NoCharge;
	NoItemPA = ptr->NoItemPA; //pour les charges proportionnelles au volume
	Qte = ptr->Qte;
	QtePhase = ptr->QtePhase;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
CL_Espece::CL_Espece(CL_Espece* source) {
	Nom = "";
	memcpy(Phase, source->Phase, sizeof(Phase));
	DebAmor = source->DebAmor;
	DurAmor = source->DurAmor;
	NbPhase = source->NbPhase;
	Supprimable = true;
}
...
CL_Atelier::CL_Atelier() {
	...
	V_ProdPied = new CL_Vecteur<CL_ItemParAtelier>(); //280307
	V_ChPied = new CL_Vecteur<CL_ItemParAtelier>();
	memset(PiedHa, 0, sizeof(PiedHa));
...
CL_Atelier::CL_Atelier(const CL_Atelier *atelier) {
	...
	V_ProdPied = new CL_Vecteur<CL_ItemParAtelier>(atelier->V_ProdPied); //280307
	V_ChPied = new CL_Vecteur<CL_ItemParAtelier>(atelier->V_ChPied);
	memcpy(PiedHa, atelier->PiedHa, sizeof(PiedHa));
...
CL_Valeur::CL_Valeur() {
	Type = 0; //valeur->Type;
	No = 0; //valeur->No;
	NoAtelier = 0; //valeur->NoAtelier;
	NoItem = 0; //valeur->NoItem;
	Atelier = NULL; //valeur->Atelier;
	Item = NULL; //valeur->Item;
	memset(Valeur, 0, sizeof(Valeur));
}
//-----------------------------------------------------------
CL_Valeur::CL_Valeur(CL_Atelier *atelier) {
	Type = 0; //valeur->Type;
	No = 0; //valeur->No;
	NoAtelier = 0; //valeur->NoAtelier;
	NoItem = 0; //valeur->NoItem;
...
//    ValInv=0;         //260903
	...
	for (n = 0; n < N_X; n++) {
		...
		AAcheter[n] = 0; //14 10 04 non stocké
		PrixAchat[n] = 0; //14 10 04 non stocké
...
//    ValInv= invAni->ValInv; //260309
	...
	memcpy(EffMax, invAni->EffMax, sizeof(float) * N_X);
	...
	memcpy(NoAtelier, invAni->NoAtelier, sizeof(int) * N_X);
	for (n = 0; n < N_X; n++)
		Atelier[n] = invAni->Atelier[n];
}
//-----------------------------------------------------------
...
CL_Troupeau::CL_Troupeau(CL_Categorie *catBestiaux) {
	...
	for (i = 0; i < V_Bestiaux->Nbelt; i++) {
		...
		if (bestiaux->Categorie != catBestiaux)
			continue;

		invAni = new CL_InvAni();
		invAni->Animal = bestiaux;
		invAni->NoAnimal = bestiaux->No;
		if (bestiaux->DonnePcent[0] != 0)
			invAni->Reproducteur = 'O';
		else
			invAni->Reproducteur = 'N';
		for (n = 0; n < N_X; n++) {
			invAni->PrixVenteMoy[n] = bestiaux->Prix;
			invAni->ValInvMoy[n] = bestiaux->ValInv;
		}
		invAni->ValInvMoy[N_X] = bestiaux->ValInv;
		V_InvAni->ins(invAni);
	}
	LstMvtAni = new TList;
...
//-----------------------------------------------------------
CL_EmpLT::CL_EmpLT(const CL_EmpLT *emp) {
	Supprimable = true;
	No = emp->No;
	Nom = emp->Nom;
	Flag = emp->Flag;
...
CL_EmpCT::CL_EmpCT(const CL_EmpCT *emp) {
	Nom = emp->Nom;
	Montant = emp->Montant;
	Taux = emp->Taux;
	Type = emp->Type; //ca s t m va...
	AReal = emp->AReal;
	MReal = emp->MReal;
	ARemb = emp->ARemb;
	MRemb = emp->MRemb;
...
//-----------------------------------------------------------
CL_Sub::CL_Sub(const CL_Sub *source) {
	No = source->No;
	Nom = source->Nom;
...
CL_Plact::CL_Plact(const CL_Plact *plact) {
	Montant = plact->Montant;
	Taux = plact->Taux;
	AReal = plact->AReal;
	MReal = plact->MReal;
	ATerme = plact->ATerme;
...
CL_Occc::CL_Occc(const CL_Occc *occc) {
	Ex = occc->Ex;
	Montant = occc->Montant;
	Taux = occc->Taux;
	Pcent = occc->Pcent;
	Frais = occc->Frais;
}
...
CL_FN_Xp1::CL_FN_Xp1(const CL_FN_Xp1 *source) {
	memcpy(Valeur, source->Valeur, sizeof(float) * (N_X + 1));
}
//-----------------------------------------------------------
CL_Immo::CL_Immo() {
	Nom = "";
...
CL_Immo::CL_Immo(const CL_Immo *immo) {
	Nom = immo->Nom;
	ValAchat = immo->ValAchat;
	AAchat = immo->AAchat;
	MAchat = immo->MAchat;
	AVente = immo->AVente;
	MVente = immo->MVente;
	TypeAmor = immo->TypeAmor;
	Duree = immo->Duree;
	ValVente = immo->ValVente;
	ValComptable = immo->ValComptable;
	NoTva = immo->NoTva;
	Tva = immo->Tva;
}
//-----------------------------------------------------------
...
CL_Petit::CL_Petit(const CL_Petit *petit) //A
		{
	ValAchat = petit->ValAchat;
	NoTva = petit->NoTva;
	Tva = petit->Tva;
	Repart = petit->Repart;
	NoRepart = petit->NoRepart;
}
//-----------------------------------------------------------
CL_TotSub::CL_TotSub() {
...
//-----------------------------------------------------------
CL_TotEmp::CL_TotEmp(const CL_TotEmp *totEmp) {
	int n;
	for (n = 0; n < N_X; n++) {
...
//-----------------------------------------------------------
CL_Prog::CL_Prog(float valeur) {
	int i;
	Arbre = new CL_Arbre(1, 10);
	Arbre->LesBranches[0].code = NOMBRE;
	for (i = 0; i < 3; i++)
		Arbre->LesBranches[0].arg[i] = -1;
	Arbre->LesBranches[0].type = NUM;

	Arbre->LesBranches[0].valeur = valeur;
	Arbre->BrancheX = 1;
...
CL_AgriRes::CL_AgriRes() {
	try {
		...
		//191205
		V_VenteAni = new CL_Vecteur<CL_QuanVal>();
		V_AchatAni = new CL_Vecteur<CL_QuanVal>();
	...
	EmpTotalPrive = new CL_TotEmp(); //020205
...
CL_AgriRes::CL_AgriRes(CL_AgriRes *source) {
	...
	EmpTotalPrive = new CL_TotEmp(source->EmpTotal); //020205
...
// a compléter
	...
	delete EmpTotal;
	EmpTotal = NULL;
	delete EmpTotalPrive;
	EmpTotalPrive = NULL; //020205
...
// en cas de variant de sumulation sur + de 10 ans je ne copie que les 10 premières années
//---------------------------------------------------------------------------
...
//CL_MvtAni *mvtAni0,*mvtAni;
	...
	EstPlus10Ans = agri->EstPlus10Ans; //false;260908                       //copie à faire
//    EstPlus10Ans    =false;
	NoVariante = agri->NoVariante;
	NbVariante = 0;
	DefSim = new CL_AgriDefSim(agri->DefSim);
//    DefSim->NbAnSim=10;          //260908
	Pere = agri->Pere;
	Resultat = NULL; //040507
	EstVariante = agri->EstVariante;
	Pere = agri->Pere;
	NoSerie = agri->NoSerie; //0;260908
...
//    LstSerie        =NULL;   //251006
//    /*
	if (agri->LstSerie == NULL) //250908
		LstSerie = NULL; //251006
	else {
		LstSerie = new TList;
		for (n = 0; n < agri->LstSerie->Count; n++) {
			noAgri = new ST_NoAgri;
			noAgri->No = 0;
			noAgri->Agri = NULL; //voir + tard cf DefAgriApres
			LstSerie->Add(noAgri);
		}
	}
//    */
...
//Nom
//an_debut   si n decades
//variante   si
//
//---------------------------------------------------------------------------
AnsiString CL_Agriculteur::NomV(void) //071008
		{
	AnsiString nom;

	if (EstPlus10Ans == true)
		nom = Nom + DefSim->An_0;
	else
		nom = Nom;
	if (EstVariante == true)
...
//Nom variante critères et alea
//---------------------------------------------------------------------------
AnsiString CL_Agriculteur::NomVC(void) {
	AnsiString nom, asAn_0;
	CL_Item *critere;
	...
	nom += "  ";
	if (DefSim->PrixProd)
		nom += DefSim->PrixProd->Nom + "  ";
	if (DefSim->PrixCharge)
		nom += DefSim->PrixCharge->Nom + "  ";
	if (DefSim->QProd)
		nom += DefSim->QProd->Nom + "  ";
	if (DefSim->QCharge)
		nom += DefSim->QCharge->Nom + "  ";
	if (DefSim->QExt)
		nom += DefSim->QExt->Nom;
...
/*
 //---------------------------------------------------------------------------
 AnsiString CL_Agriculteur::NomV(void)  //230908
 {
 AnsiString nom,asAn_0;
...
/*
 ...
 if(LstVariante==NULL  || LstVariante->Count==0 || NoVariante==0)
 ; //ne rien faire
 else
 nom= nom+"  "+IntToStr(NoVariante);
 ...
 nom+="  ";
 if(DefSim->PrixProd)  nom+=DefSim->PrixProd->Nom+"  ";
 if(DefSim->PrixCharge)nom+=DefSim->PrixCharge->Nom+"  ";
 if(DefSim->QProd)     nom+=DefSim->QProd->Nom+"  ";
 if(DefSim->QCharge)   nom+=DefSim->QCharge->Nom+"  ";
 if(DefSim->QExt)      nom+=DefSim->QExt->Nom;
...
void CL_Agriculteur::SupMaFalse(void) {
	...
	for (i = 0; i < LstTroupeaux->Count; i++) //131003
		...
		for (j = 0; j < troupeau->V_InvAni->Nbelt; j++) {
			...
			if (invAni == NULL)
				continue; //171007
			if (invAni->Animal == NULL)
				continue; //171007
			invAni->Animal->Supprimable = false;
			{
				for (n = 0; n < N_X; n++) {
...
//---------------------------------------------------------------------------
CL_Elt::~CL_Elt(void) {
//    if(Pcent!=NULL){ delete []Pcent;Pcent=NULL;}  //260905
//    if(Passe){ delete[]Passe;Passe=NULL;}
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
CL_Elt::CL_Elt(CL_Poste *poste, CL_Calendrier *calendrier) //0900306
		{
	Maz();
	NoFamille = poste->No;
	NoItem = calendrier->No;
	Poste = poste;
	Nom = calendrier->Nom.c_str();
	NomFamille = poste->Nom.c_str();
	NomCat = poste->Nom.c_str();

	Valeur = calendrier->TotBesoins; //110306
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
CL_Elt::CL_Elt(int noFamille, char *nomNature, char *nomFamille, char *nomCat,
	...
	NoCategorie = noCat;
	Nom = nom;
	NoItem = noItem; //attention dance cas ce n'est pas tjrs Item Atelier...
	Valeur = valeur;
}
//---------------------------------------------------------------------------
void CL_Elt::Maz(void) {
	No = 0;
	Flag = ' ';
	NoFamille = 0; //PRODUIT CHARGE RECDIV...INDIC  cf olympedec.h
	NoCategorie = -1; //seult pour produit charge
	NoType = 0; //seult pour animaux EFF EFFV EFFA VALV VALA VALINV
	NoItem = 0;
	NoEspece = 0;
	NoEtat = 0;
	Poste = NULL;
...
CL_Delta::CL_Delta() {
	...
	for (int n = 0; n < 100; n++)
		Pcent[n] = 100; //210907
	//memset(Pcent,100,sizeof(Pcent));
}
CL_Delta::CL_Delta(const CL_Delta *delta) {
	NMax = delta->NMax;
	Item = delta->Item;
	NoItem = delta->NoItem;
	Atelier = delta->Atelier;
	NoAtelier = delta->NoAtelier;
	Nature = delta->Nature;
	Rdt = delta->Rdt;
//    memcpy(Pcent,delta->Pcent,sizeof(Pcent));//210907
	for (int n = 0; n < 100; n++)
		Pcent[n] = delta->Pcent[n]; //210907
...
//------------------ ------------------------------------------------------------
CL_AgriDefSim::~CL_AgriDefSim(void) {
}
//------------------ ------------------------------------------------------------
...
//------------------ ------------------------------------------------------------
CL_MvtAni::CL_MvtAni(int r, int date, int type, int eff, float info) {
	Ligne = r;
	Type = type;
	Date = date;
	Eff = eff;
...
void CL_Repart::GrilleEcrire(TStringGrid* sg, float valTot) {
	...
	CL_IntFloat *moisVal;
	int precision;
	if (FenRepartFrm->RGPrecision->ItemIndex == 1)
		precision = 2;
	else
		precision = 0;
	...
	for (i = 0; i < LstPcent->Count; i++) {
		...
		sg->Cells[c][r] = Aff(valTot * moisVal->Val / 100., precision);
	}
}
//---------------------------------------------------------------------------
