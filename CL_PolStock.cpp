...
void CL_PolStock::EcrireFichier(FILE *fichier) {
	...
	fwrite(&PcentStock, sizeof(float), 1, fichier);
	fwrite(&Maxi, sizeof(float), 1, fichier);
	fwrite(&PcentDeltaPrix, sizeof(float), 1, fichier);
	fwrite(&PcentDecote, sizeof(float), 1, fichier);
}
//---------------------------------------------------------------------------
void CL_PolStock::LireFichier(FILE *fichier) {
	fread(&No, sizeof(int), 1, fichier);
	Item = V_Produit->trouve(No);
	fread(&PcentStock, sizeof(float), 1, fichier);
	fread(&Maxi, sizeof(float), 1, fichier);
	fread(&PcentDeltaPrix, sizeof(float), 1, fichier);
	fread(&PcentDecote, sizeof(float), 1, fichier);
}
/*
 A NOTER
